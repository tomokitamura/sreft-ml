...
def save_csv_with_hash_demo(
    df: pd.DataFrame, params: pd.DataFrame, file_df: str, file_params: str
) -> None:
    """
    Save DataFrames to CSV files with a hash value appended to the filenames.

    Args:
        df (pd.DataFrame): DataFrame to be saved.
        params (pd.DataFrame): Parameters DataFrame to be saved.
        file_df (str): Filepath for saving the main DataFrame with hash value appended.
        file_params (str): Filepath for saving the parameters DataFrame with hash value appended.
    ...
    file_df = file_df + "_" + hash_val + ".csv"
    file_params = file_params + "_" + hash_val + ".csv"
    df.to_csv(file_df, na_rep="NA", index=False, lineterminator="\n", mode="x")
    params.to_csv(file_params, na_rep="NA", index=False, lineterminator="\n", mode="x")
    print("the file is saved as " + file_df + " and " + file_params)
...
def data_synthesis(
    ...
    surv_inv: Callable[[float, dict], float],
    params_surv: dict,
...
df, params_output = data_synthesis(
    rng_seed=42,
    n_biomarker_total=15,
    n_biomarker_noise=3,
    n_covariate_total=3,
    n_covariate_noise=1,
    n_subject=5000,
    ...
    surv_inv=surv_inv,
    params_surv={"lambda_": 0.015, "c_": 0.25},
)
